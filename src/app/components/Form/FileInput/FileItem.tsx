...
const fileItem = tv({
	slots: {
		...
		icon: 'rounded-full border-4 border-violet-100 bg-violet-2020 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500',
		deleteButton: ''
	},
			...
			{state === 'error' ? (
				<div className="flex flex-1 flex-col items-start gap-1">
					<div className="flex flex-col">
						<span className="text-sm font-medium text-red-700 dark:text-error-400">Upload failed. Please, try again.</span>
						<span className="text-sm text-red-600 dark:text-error-500">{name}</span>
					</div>
				...
				<div className="flex flex-1 flex-col items-start gap-1">
					...
					<div className="flex w-full items-center gap-3">
						...
						<span className="text-sm font-medium text-zinc-700 dark:text-violet-300">{state === 'complete' ? '100%' : '80%'}</span>
					</div>
				</div>
			)}
